...
const fileToBase64String = (file) => {
  return new Promise((resolve, reject) => {
    // Zachary - error checks
    if (!file) {
      return reject(new Error(D23_005));
    }
    // Zachary - load the file
...
class PictureForm extends React.Component {
  constructor(props) {
    super(props);
    this.emptyState = {
      preview: undefined, // Zachary - base64 representation of the picture we'll upload
      picture: '', // Zachary -  this will be a path, used for the backend
      description: '',
    };
    this.state = this.emptyState;

    autoBind.call(this, PictureForm); // Zachary - this is not a react/redux convention
  }
  //-------------------------------------------------------------------
  // Member functions
  //-------------------------------------------------------------------
  handleChange(event) {
    ...
    } else { // Zachary - I must be working in the descrpition
      this.setState({
        description: value,
      });
    }
